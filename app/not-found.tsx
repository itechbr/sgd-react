...
// 2. Importação Dinâmica do componente de animação
// Isso separa o código pesado do resto da página e permite mostrar um Skeleton enquanto carrega
const NotFoundAnimation = dynamic(
  () => import('./components/ui/NotFoundAnimation'),
  { 
    ssr: false, // Desabilita renderização no servidor (Lottie é client-side)
    loading: () => (
      // Este é o Skeleton que aparece instantaneamente nos primeiros 2 segundos
      <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center bg-gray-100/50 rounded-full animate-pulse">
        <Loader2 className="w-10 h-10 text-gray-300 animate-spin mb-2" />
        <span className="text-xs text-gray-400 font-mono">CARREGANDO SISTEMA...</span>
      </div>
    )
  }
)
    ...
    <div className="min-h-screen w-full bg-black flex items-center justify-center p-4 md:p-8 font-sans selection:bg-[#C0A040] selection:text-black">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-6xl bg-[#121212] rounded-[2rem] overflow-hidden shadow-2xl shadow-[#C0A040]/5 border border-[#333333] relative">
        ...
        <div className="w-full lg:w-1/2 bg-gray-50 flex flex-col items-center justify-center p-12 relative overflow-hidden group">
          ...
          {/* Wrapper da animação com tamanho fixo ou proporcional para evitar pulos de layout */}
          <div className="relative z-10 w-full max-w-md aspect-square transform transition-transform duration-700 hover:scale-105 flex items-center justify-center">
            <NotFoundAnimation />
          </div>
